...
3. **Performer & model/effort per unit — two-stage routing** *(judgment)*: decide each unit in **two stages** — **(a)** pick the **performer cell** {codex-session | claude-subagent | conductor-direct}, then **(b)** pick **model/effort within that cell** (codex = the roster routing below, unchanged; claude = a per-call haiku/sonnet/opus choice — **no reasoning-effort knob, effort flags are codex-only**; conductor-direct = Fable inline, no model knob). Default cell is **codex** (separate token pool); claude/conductor-direct are chosen only on the signals below. **Primary persona is the dual-Max heavy user** (deep quota on BOTH pools): with both pools healthy, the deciding axis is **task-fit** (reasoning-vs-throughput, context-depth, parallelism) — not frugality; budget signals enter the decision only when a pool actually shows pressure.

   **Stage (a) — performer cell** *(judgment — prose heuristics, not a scorecard)*:
   - **conductor-direct (Fable inline)** — trivial · fully-specified · single-file/one-liner units: this is step 0's cheap path re-cast as the matrix's third cell (when the WHOLE task is just this one unit, keep step 0's ceremony/ledger skip — see step 0). Also: a unit of a multi-unit run where the conductor already holds the full mental model, so the **context re-transfer cost of dispatch would exceed the work itself**; and latency-critical tiny edits. **No parallelism** (single conductor thread) — never herd multiple units here. **Single-file is a real boundary, not a vibe**: multi-file mechanical work (renames, boilerplate sweeps), however trivial per-file, routes to a light codex model at low effort instead — it offloads to the separate pool for near-zero quota cost and keeps the conductor's budget free; conductor-direct absorbs it only when the context-re-transfer clause genuinely applies.
...
- **Native task-tree visibility** *(mechanics)*: the background dispatch call's display label/description MUST be `codex <model> · <unit-slug>` (e.g. `codex gpt-5.6-sol · auth-api`) — codex performers then appear as rows in Claude Code's own running-agents tree alongside claude subagents, exactly like the maestro-workflows mixed view. An `events --follow` observer uses `codex <model> · <unit-slug> (observe)`. The row tracks the transport process, not the server-side turn: on msg timeout the row ends while the turn continues — the events row and `read` polling cover that gap.
